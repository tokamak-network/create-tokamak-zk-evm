    ...
    command
        ...
        try {
            if (options.interactive) {
                ...
                if (!proofFile) {
                    throw new Error('Proof file path is required. Use --interactive for interactive mode.');
                }
                await verifyProof(proofFile, options);
            }
        }
        catch (error) {
            logger_1.logger.error('Proof verification failed:', error);
            process.exit(1);
        }
    });
    return command;
}
async function verifyProof(proofFile, options) {
    const { verbose = false } = options;
    logger_1.logger.info(chalk_1.default.blue(`🔍 Verifying proof: ${proofFile}`));
    ...
    try {
        ...
        const result = await scriptRunner.runVerify({
            ...
            onProgress: (step, _current, _total) => {
                logger_1.logger.info(`Verification: ${step}`);
            },
        });
        console.log();
        if (result.success) {
            // Check actual verification result from the output file
            const verificationResult = await getVerificationResult(binaryPaths.resourceDir);
            if (verificationResult && verificationResult.verified === true) {
                console.log(chalk_1.default.green('✅ Proof format is VALID'));
                console.log(chalk_1.default.green('✅ Verification result: TRUE'));
            }
            else if (verificationResult && verificationResult.verified === false) {
                console.log(chalk_1.default.green('✅ Proof format is VALID'));
                console.log(chalk_1.default.red('❌ Verification result: FALSE'));
            }
            else {
                console.log(chalk_1.default.yellow('⚠️ Verification completed but result is unclear.'));
            }
            if (proofInfo) {
            ...
            console.log(chalk_1.default.red('❌ Proof verification script failed to execute!'));
...
async function setupProofForVerification(proofDir, resourceDir) {
    ...
    for (const type of outputTypes) {
        ...
        if (await fs.pathExists(sourceDir)) {
            ...
            logger_1.logger.debug(`Copied ${type} files for verification`);
        }
    }
}
function displayProofInfo(proofInfo) {
    console.log();
    console.log(chalk_1.default.blue('📋 Proof Information'));
    console.log(chalk_1.default.gray('===================='));
    if (proofInfo.txHash) {
        console.log(`Transaction Hash: ${chalk_1.default.cyan(proofInfo.txHash)}`);
    }
    if (proofInfo.timestamp) {
        console.log(`Generated: ${chalk_1.default.cyan(new Date(proofInfo.timestamp).toLocaleString())}`);
    }
    if (proofInfo.network) {
        console.log(`Network: ${chalk_1.default.cyan(proofInfo.network)}`);
    }
    if (proofInfo.outputs) {
        ...
        for (const [type, files] of Object.entries(proofInfo.outputs)) {
            if (Array.isArray(files) && files.length > 0) {
                console.log(`  ${type}: ${chalk_1.default.gray(files.join(', '))}`);
            }
        }
    }
}
async function displayVerificationResult(resourceDir) {
    const verificationResultPath = path.join(resourceDir, 'verify', 'output', 'verification.json');
    if (await fs.pathExists(verificationResultPath)) {
        try {
            const result = await fs.readJson(verificationResultPath);
            console.log();
            console.log(chalk_1.default.blue('🔍 Verification Details'));
            console.log(chalk_1.default.gray('======================='));
            if (result.verified !== undefined) {
                const status = result.verified
                    ? chalk_1.default.green('✅ Valid')
                    : chalk_1.default.red('❌ Invalid');
                console.log(`Proof Status: ${status}`);
                const verificationResult = result.verified
                    ? chalk_1.default.green('TRUE')
                    : chalk_1.default.red('FALSE');
                console.log(`Verification Result: ${verificationResult}`);
            }
            if (result.timestamp) {
                console.log(`Verified At: ${chalk_1.default.cyan(new Date(result.timestamp).toLocaleString())}`);
            }
            if (result.exitCode !== undefined) {
                console.log(`Exit Code: ${chalk_1.default.gray(result.exitCode)}`);
            }
            if (result.message) {
                console.log(`Message: ${chalk_1.default.gray(result.message)}`);
            }
        }
        catch (error) {
            logger_1.logger.debug('Could not parse verification result:', error);
        }
    }
}
/**
...
async function verifyInteractive(options) {
    ...
    logger_1.logger.info(chalk_1.default.blue('🔍 Interactive Proof Verification'));
    console.log();
    // Get list of proof outputs
    const proofOutputs = await getProofOutputs(outputDir);
    if (proofOutputs.length === 0) {
        console.log(chalk_1.default.yellow('No proof outputs found.'));
        console.log(chalk_1.default.gray('Run a proof generation first with: tokamak-zk-evm prove <tx-hash>'));
        return;
    }
    // Display available proofs
    console.log(chalk_1.default.yellow(`Found ${proofOutputs.length} proof output(s):`));
    console.log();
    proofOutputs.forEach((proof, index) => {
        console.log(`${chalk_1.default.cyan(`${index + 1}.`)} ${chalk_1.default.white(proof.name)}`);
        console.log(`   ${chalk_1.default.gray('Created:')} ${proof.created.toLocaleString()}`);
        if (proof.summary?.txHash) {
            console.log(`   ${chalk_1.default.gray('Transaction:')} ${proof.summary.txHash}`);
        }
        if (proof.summary?.network) {
            console.log(`   ${chalk_1.default.gray('Network:')} ${proof.summary.network}`);
        }
        console.log();
    });
    ...
    console.log(chalk_1.default.blue(`Selected: ${selectedProof.name}`));
    console.log();
 ...
 */
async function verifyWithRegeneration(proofPath, options) {
    logger_1.logger.info(chalk_1.default.blue('🔄 Regenerating and Verifying Proof'));
    console.log();
    ...
    if (!isValidTxHash(txHash)) {
        throw new Error(`Invalid transaction hash in transaction_hash.txt: ${txHash}`);
    }
    logger_1.logger.info(`Transaction Hash: ${chalk_1.default.cyan(txHash)}`);
    // Load configuration
    const configManager = new config_manager_1.ConfigManager();
    const config = await configManager.loadConfig();
    // Ensure binary is available
    ...
    const spinner = (0, ora_1.default)('Regenerating proof components...').start();
    ...
    try {
        ...
        const proveOutputDir = path.join(binaryPaths.resourceDir, 'prove', 'output');
        await fs.ensureDir(proveOutputDir);
        await fs.copy(proofJsonPath, path.join(proveOutputDir, 'proof.json'));
        spinner.succeed(chalk_1.default.green('✅ Proof components regenerated successfully!'));
        ...
        spinner.fail(chalk_1.default.red('❌ Regeneration failed'));
        ...
        if (error instanceof Error && error.message.includes('SETUP_MISSING')) {
            ...
            console.log(chalk_1.default.gray('====================================='));
            console.log();
            console.log(chalk_1.default.yellow('The verification failed because trusted setup files are missing.'));
 ...
 */
async function getProofOutputs(outputDir) {
 ...
 */
async function promptForProofSelection(maxIndex) {
    ...
    return new Promise((resolve) => {
        const askForSelection = () => {
            rl.question(chalk_1.default.yellow(`Select a proof to verify (1-${maxIndex}): `), (answer) => {
                const selection = parseInt(answer.trim(), 10);
                if (isNaN(selection) || selection < 1 || selection > maxIndex) {
                    console.log(chalk_1.default.red(`Please enter a number between 1 and ${maxIndex}`));
                    askForSelection();
                }
                else {
                    rl.close();
 ...
 */
async function getVerificationResult(resourceDir) {
    const verificationResultPath = path.join(resourceDir, 'verify', 'output', 'verification.json');
    if (await fs.pathExists(verificationResultPath)) {
        try {
            const result = await fs.readJson(verificationResultPath);
